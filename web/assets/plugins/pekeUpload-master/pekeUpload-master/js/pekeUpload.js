...
(function($) {
    ...
    $.fn.pekeUpload = function(options) {
        ...
        var defaults = {
            ...
            allowedExtensions: "",
            invalidExtError: "Type de fichier invalide !",
            maxSize: 0,
            sizeError: "La taille du fichier est trop volumineuse !",
        ...
        this.each(function() {
            ...
            if (options.theme == "bootstrap" && options.isLargeBtn == true) {
                var html = '<a href="javascript:void(0)" class="btn btn-primary btn-upload btn-lg"> <span class="icon-upload icon-white"></span> ' + options.btnText + '</a><div class="pekecontainer"></div>';
            }
            if (options.theme == "bootstrap" && options.isLargeBtn == false) {
        ...
        function UploadFile() {
            ...
            if (options.theme == "bootstrap") {
                var htmlprogress = '<div class="file"><div class="filename"></div><div class="progress progress-striped"><div class="bar progress-bar" style="width: 0%;"><span class="badge badge-info"></span></div></div></div>';
            }
            if (options.theme == "custom") {
                var htmlprogress = '<div class="file"><div class="filename"></div><div class="progress-pekeupload"><div class="bar-pekeupload progress-bar" style="width: 0%;"><span></span></div></div></div>';
            }
            ...
            $.ajax({
                ...
                success: function(data) {
                    var percent = 100;
                    obj.next('a').next('div').find('.progress-bar:first').width(percent + '%');
                    obj.next('a').next('div').find('.progress-bar:first').text(percent + "%");
                    if (data == 1) {
                        ...
                        if ((options.theme == "bootstrap") && (options.showErrorAlerts == true)) {
                            obj.next('a').next('div').prepend('<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button> ' + data + '</div>');
        ...
        function progressHandlingFunction(e) {
            if (e.lengthComputable) {
                ...
                if (options.showPercent == true) {
                    ...
                    obj.next('a').next('div').find('.file').first().find('.progress-bar:first').width(percent + '%');
                }
                obj.next('a').next('div').find('.file').first().find('.progress-bar:first').html('<center>' + percent + "%</center>");
            }
        }
        //Validate master
        function validateresult() {
            ...
            if (options.allowedExtensions != "") {
                ...
                if (validationresult == false) {
                    canUpload = false;
                    if ((options.theme == "bootstrap") && (options.showErrorAlerts == true)) {
                        obj.next('a').next('div').prepend('<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button> ' + options.invalidExtError + '</div>');
                        bootstrapclosenotification();
                    }
                    if ((options.theme == "custom") && (options.showErrorAlerts == true)) {
            ...
            if (options.maxSize > 0) {
                ...
                if (validationresult == false) {
                    canUpload = false;
                    if ((options.theme == "bootstrap") && (options.showErrorAlerts == true)) {
                        obj.next('a').next('div').prepend('<div class="alert alert-danger"><button type="button" class="close" data-dismiss="alert">&times;</button> ' + options.sizeError + '</div>');
                        bootstrapclosenotification();
                    }
                    if ((options.theme == "custom") && (options.showErrorAlerts == true)) {
        ...
        function bootstrapclosenotification() {
            obj.next('a').next('div').find('.alert-danger').click(function() {
                $(this).remove();
            });
        }
